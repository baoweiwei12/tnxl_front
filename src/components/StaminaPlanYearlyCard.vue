<template>
    <div style="text-align: center;margin-bottom: 10px;font-size: larger;">
        <span>{{ `${planData.full_name} ${planData.year}年 年度体能训练计划` }}</span>
    </div>
        ...
        <el-card v-for="(dayPlan, index) in planData.plan" style="max-width: 500px">
            <template #header>
                <div class="card-header">
                    <span>{{ planData.year }}年{{ index + 1 }}月</span>
                </div>
            </template>
                ...
                <el-table-column label="科目" prop="subject" width="110">
                    <template #default="scope">
                        <el-select size="small" v-model="scope.row.subject" placeholder="请选择训练科目">
                            <el-option v-for="item in ['三公里', '单杠', '仰卧起坐', '蛇行跑', '双杠', '负重', '木马']" :key="item"
...
import { editStaminPlanYearly } from '@/api';
...
const handleSave = async () => {
    try {
        ...
        await editStaminPlanYearly(planData.value.id, planData.value)
