<template>
    <div style="text-align: center;margin-bottom: 10px;font-size: larger;">
        <span>{{ `${planData.full_name} ${planData.year}年${planData.month}月 月度专业训练计划` }}</span>
    </div>
...
import { editMajorPlanMonthly } from '@/api';
...
const handleSave = async () => {
    try {
        ...
        await editMajorPlanMonthly(planData.value.id, planData.value)
