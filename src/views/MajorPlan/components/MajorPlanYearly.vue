...
import { getMajorPlanYearly, addMajorPlanYearly, deleteMajorPlanYearly } from '@/api';
...
const handleAddMajorPlanSubmit = async (form) => {
    try {
        ...
        await addMajorPlanYearly(user_id, year)
        await fetchPlans()
...
const fetchPlans = async () => {
    try {
        loading.value = true
        const resp = await getMajorPlanYearly({
...
const handleDelete = async (row) => {
    try {
        await deleteMajorPlanYearly(row.id)
