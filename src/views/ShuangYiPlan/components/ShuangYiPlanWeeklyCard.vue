<template>
    <div style="text-align: center;margin-bottom: 10px;font-size: larger;">
        <span>{{ `${planData.full_name} ${planData.year}年${planData.month}月 第${planData.week}周 双一训练计划` }}</span>
    </div>
...
import { editShuangYiPlanWeekly } from '@/api';
...
const handleSave = async () => {
    try {
        ...
        await editShuangYiPlanWeekly(planData.value.id, planData.value)
