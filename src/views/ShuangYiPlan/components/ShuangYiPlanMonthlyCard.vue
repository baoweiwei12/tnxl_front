<template>
    <div style="text-align: center;margin-bottom: 10px;font-size: larger;">
        <span>{{ `${planData.full_name} ${planData.year}年${planData.month}月 月度双一训练计划` }}</span>
    </div>
...
import { editShuangYiPlanMonthly } from '@/api';
...
const handleSave = async () => {
    try {
        ...
        await editShuangYiPlanMonthly(planData.value.id, planData.value)
