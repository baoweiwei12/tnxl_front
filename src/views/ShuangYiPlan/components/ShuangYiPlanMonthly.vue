<template>
    <div style="display: flex;gap: 10px; margin-bottom: 10px;"> <el-input v-model="queryFullName" placeholder="姓名"
        ...
        <el-button @click="addShuangYiPlanDialogVisible = true" style="width: 100px;">添加月计划</el-button>
    </div>


    <ShuangYiPlanMonthlyTable :data="data" :loading="loading" @edit="handleEdit" @delete="handleDelete">
    </ShuangYiPlanMonthlyTable>
    <el-dialog title="添加周计划" v-model="addShuangYiPlanDialogVisible">
        <ShuangYiPlanMonthlyAddForm @submit="handleAddShuangYiPlanSubmit" />
    </el-dialog>

    <el-dialog fullscreen v-model="editDialogVisible">
        <ShuangYiPlanMonthlyCard :data="currentPlan">
        </ShuangYiPlanMonthlyCard>
    </el-dialog>
</template>
<script setup>
import { ref, onMounted } from 'vue'
import { getShuangYiPlanMonthly, addShuangYiPlanMonthly, deleteShuangYiPlanMonthly } from '@/api';

import ShuangYiPlanMonthlyAddForm from './ShuangYiPlanMonthlyAddForm.vue';
import ShuangYiPlanMonthlyCard from './ShuangYiPlanMonthlyCard.vue';
import ShuangYiPlanMonthlyTable from './ShuangYiPlanMonthlyTable.vue';
...
const handleAddShuangYiPlanSubmit = async (form) => {
    try {
        ...
        await addShuangYiPlanMonthly(user_id, year, month, week)
        await fetchPlans()
...
const fetchPlans = async () => {
    try {
        loading.value = true
        const resp = await getShuangYiPlanMonthly({
...
const handleDelete = async (row) => {
    try {
        await deleteShuangYiPlanMonthly(row.id)
