<template>
    <div style="text-align: center;margin-bottom: 10px;font-size: larger;">
        <span>{{ `${planData.full_name} ${planData.year}年 年度双一训练计划` }}</span>
    </div>
...
import { editShuangYiPlanYearly } from '@/api';
...
const handleSave = async () => {
    try {
        ...
        await editShuangYiPlanYearly(planData.value.id, planData.value)
