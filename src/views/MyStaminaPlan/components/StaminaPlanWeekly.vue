<template>
    ...
    <StaminaPlanWeeklyTable :data="data" :loading="loading" @edit="handleEdit">
    </StaminaPlanWeeklyTable>


    <el-dialog fullscreen v-model="editDialogVisible">
        <StaminaPlanWeeklyCard :data="currentPlan"></StaminaPlanWeeklyCard>
    </el-dialog>
</template>
<script setup>
import { ref, onMounted } from 'vue'
import { getMyStaminaPlanWeekly } from '@/api';
...
const fetchPlans = async () => {
    try {
        loading.value = true
        const resp = await getMyStaminaPlanWeekly({
