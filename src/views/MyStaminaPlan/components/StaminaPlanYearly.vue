<template>
    <div style="display: flex;gap: 10px; margin-bottom: 10px;">
        ...
        <el-button @click="addStaminaPlanDialogVisible = true" style="width: 100px;">添加年计划</el-button>
    </div>

    <StaminaPlanYearlyTable :data="data" :loading="loading" @edit="handleEdit" @delete="handleDelete" />

    <el-dialog title="添加周计划" v-model="addStaminaPlanDialogVisible">
        <StaminaPlanYearlyAddForm @submit="handleAddStaminaPlanSubmit" />
    </el-dialog>
...
import { getMyStaminaPlanYearly } from '@/api';
...
const fetchPlans = async () => {
    try {
        loading.value = true
        const resp = await getMyStaminaPlanYearly({
